...
image:ontap-cyber-vault-connections.png["Stellen Sie sicher, dass keine Verbindungen zum Cyber-Tresor bestehen"]

Dieser Screenshot zeigt, dass die Snapshots nicht manipuliert werden können.

image:ontap-cyber-vault-tamperproof-snapshots.png["Stellen Sie sicher, dass Snapshot-Kopien nicht manipuliert werden können."]

Führen Sie die folgenden Schritte aus, um die Air-Gapping-Funktionalität zu validieren und zu bestätigen:

* Testen Sie die Netzwerkisolationsfunktionen und die Möglichkeit, eine Verbindung stillzulegen, wenn keine Daten übertragen werden.
